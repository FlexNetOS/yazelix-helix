...
Helix tree-sitter grammar sources are locked in `grammar_sources.lock.json` and fetched through fixed-output `fetchFromGitHub`, `fetchzip`, or `fetchgit` derivations instead of eval-time `builtins.fetchTree`, so cold evaluation stays local and grammar source builds can substitute from cache.
